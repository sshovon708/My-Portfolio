...
const logoImg = "/iftakhar-shovon-logo.webp";
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-10 lg:px-20 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <Link to="/" className="flex items-center group">
              <img
                src={logoImg}
                alt="IAShovon — Full-Stack Web Developer Logo"
          ...
          <nav className="hidden md:flex items-center gap-1">
            {navLinks.map(({ to, label }) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `relative px-4 py-2 rounded-lg text-[14px] tracking-wide transition-all duration-600 group ${
                    isActive
                      ? "text-blue-900 bg-blue-500/10 font-bold"
                      : "text-slate-900 hover:text-slate-100 hover:bg-slate-800 font-medium"
                  }`
                }
              >
                {({ isActive }) => (
                  <>
                    ...
                    <span className="absolute bottom-1 left-1/2 w-0 h-0.5 bg-white rounded-full transition-all duration-300 origin-center -translate-x-1/2 group-hover:w-3/5" />

                    {/* Click Active Slide indicator */}
                    {isActive && (
                      <span className="absolute bottom-1 left-1/2 -translate-x-1/2 w-3/5 h-0.5 rounded-full bg-blue-700 transition-all duration-300" />
                    )}
                  </>
                )}
              </NavLink>
